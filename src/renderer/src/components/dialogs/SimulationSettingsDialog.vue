<template>
  <BaseDialog header="Simulation Settings..." class="w-169">
    <div class="flex flex-col gap-4">
                  ...
                  <div class="flex flex-col gap-4">
                    <div v-for="(outputPlot, outputPlotIndex) in localUiJson.output.plots" :key="`plot_${outputPlotIndex}`">
                      <div class="flex flex-col gap-2 p-3 border rounded">
                        ...
                        <Button label="Remove Plot" icon="pi pi-trash" severity="danger" size="small" @click="removeOutputPlot(outputPlotIndex)" class="w-fit" />
                      </div>
                    </div>
                    <Button
      ...
      <div v-if="validationIssues.length > 0" class="border border-orange-300 bg-orange-50 p-3 rounded">
        <label class="font-semibold text-orange-800 mb-2">Validation Issues:</label>
        <ul class="list-disc list-inside text-sm">
          <li v-for="(issue, issueIndex) in validationIssues" :key="`issue_${issueIndex}`" class="text-orange-700">
            {{ issue.description }}
          </li>
        </ul>
      </div>
...
const emit = defineEmits(['close', 'ok', 'update:uiJson']);
...
function onOk() {
  emit('update:uiJson', localUiJson.value);
